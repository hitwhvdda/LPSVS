...
focalloss = FocalLoss()
class FastText_BiLSTM(pl.LightningModule):
    """model."""
...
def train_and_save(hidden_size,fc1_size,lr,dropout1,batch_size,max_epoch=20):
    ...
    print(config)
    model = FastText_BiLSTM(config).to(0)
    data = CCSDatasetDataModule(CCSDataset,config)
    logger=TensorBoardLogger(save_dir="/home/ao_ding/expand/trace/result", name="FastText_BiLSTM", version=".")
