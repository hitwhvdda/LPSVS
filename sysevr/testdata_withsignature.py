...
#from pl_model_word2vec import Word2vec_BRNN
...
embedding_size = 300
max_length = 55
# Word2vec_Model = Word2Vec.load("/home/ao_ding/expand/SySeVR/code/word2vec_model.model")
# glove_file = "/home/ao_ding/expand/SySeVR/code/glove.txt"
# Glove_Model = KeyedVectors.load_word2vec_format(glove_file)
fasttext_file = "/home/ao_ding/expand/SySeVR/code/fasttext_model.model"
...
def selectforlist():
    conn = sqlite3.connect('/home/ao_ding/expand/SySeVR/database/signature_fasttext_bgru.db')
    ...
    list5=c.fetchall()
    c.execute("SELECT * FROM signatures WHERE label = 6")
    list6=c.fetchall()
    c.execute("SELECT * FROM signatures")
    listall=c.fetchall()
    #print(list0[:10],list1[:10],list2[:10],list3[:10],list4[:10],list5[:10],list6[:10])
    conn.close()
...
def compare(output):
    ...
    for i in range(listlen):
        ...
        #print(eval(output))
        #print(signature)
        cos_sim = 1 - spatial.distance.cosine(eval(output), signature)
        if cos_sim>=maxsim:
            signature_filename=filename
            maxsim=cos_sim
            label_pred=label
            signature_trace = source
    signature_source=signature_filename.split('/')[-1]
...
def test_withsignature(PATH=""):
    ...
    for data,label,filename in zip(datalist,labellist,filenamelist):
        ...
        if CWEtype in worklist:
            ...
            selectfilname.append(filename)
    newdatalist=[]
    for data,filename in zip(selectdata,selectfilname):
        newdatalist.append((data,filename))
    stratify = selectlabel
    x_train,x_test,y_train,y_test = train_test_split(
        newdatalist,selectlabel,test_size=1-0.8,random_state=1,stratify=stratify
    )
    stratify = y_test
    ...
    #model = Word2vec_BRNN.load_from_checkpoint(PATH).cuda(0)
    ...
    for threshold in thresholdlist:
        ...
        for a_fv in fvlist:
            ...
            label_pred=a_fv[2]
            label=a_fv[3]
            signature_trace = a_fv[4]
            if(cos_sim>threshold and label_pred==label):
                tplist.append((signature_source,cos_sim,label_pred,signature_trace))
            elif(cos_sim>threshold and label_pred!=label):
                fplist.append((signature_source,cos_sim,label_pred,signature_trace))
            else:
                fnlist.append((signature_source,cos_sim,label_pred,signature_trace))
...
if __name__ == "__main__":
    ...
    PATH="/home/ao_ding/expand/SySeVR/result/FastText_BGRU/checkpoints/sample-epoch=19-val_acc=0.99.ckpt"
