...
embedding_size = 300
weights = torch.tensor([2786,1160,3197,5773], dtype=torch.float32).cuda(0)
...
class CCSDataset:
    ...
    def __init__(self, partition="train", max_length=MAX_LENGTH):
        """Init."""
...
class Word2vec_BiLSTM(pl.LightningModule):
    """model."""
    ...
    def __init__(self, config):
        ...
        self.fc2 = torch.nn.Linear(self.fc1_size, 4)
...
class Codebert_BGRU(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=self.dropout1)
        self.fc2 = nn.Linear(self.fc1_size, 4)
        self.accuracy = torchmetrics.Accuracy(task="multiclass", num_classes=7)
...
def train_and_save(hidden_size,fc1_size,lr,dropout1,batch_size,max_epoch=20):
    ...
    data = CCSDatasetDataModule(CCSDataset,config)
    logger=TensorBoardLogger(save_dir="/home/ao_ding/expand/SySeVR/result", name="Word2vec_BGRU", version=".")
    ...
    trainer = pl.Trainer(
        default_root_dir="/home/ao_ding/expand/SySeVR/result",
...
if __name__=='__main__':
    ...
    train_and_save(384,128,0.0001,0.3,64)
